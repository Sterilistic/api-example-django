from django.db import models

# Add your models here
from django.contrib.auth.models import User


class Doctor(models.Model):
	'''
	Contains doctor's info & doctor_id
	'''
	user = models.OneToOneField(User)
	doctor_id = models.IntegerField()

class Patient(models.Model):
	'''
	Contains Patient's demographic info
	'''
	GENDER_CHOICES = (
		('M', 'Male'),
		('F', 'Female'),
		('O', 'Other'),
	)
	gender = models.CharField(max_length=1, choices=GENDER_CHOICES, default='O')
	patient_id = models.IntegerField(unique=True)
	doctor_id = models.IntegerField()
	first_name = models.CharField(max_length=100)
	last_name = models.CharField(max_length=100)
	email = models.EmailField()

	def __str__(self):
		return '%s %s %s' % (self.first_name, self.last_name, str(self.patient_id))


class Appointment(models.Model):
	'''
	Contains Appointment details
	'''
	appointment_id = models.CharField(unique=True, max_length=100)
	patient = models.ForeignKey('Patient')
	scheduled_time = models.DateTimeField(auto_now=False, auto_now_add=False, null=True)
	doctor_id = models.IntegerField()
	time_waited = models.DurationField(null=True)
	status = models.CharField(max_length=100,null=True,default='')
	arrival_time = models.DateTimeField(auto_now=False, auto_now_add=False, default=None)

	def __str__(self):
		return '%s %s at %s' % (self.patient.first_name, self.patient.last_name, str(self.scheduled_time))


class Arrival(models.Model):
	'''
	Maps appointment with doctor_id
	'''
	appointment_id = models.CharField(unique=True, max_length=100)
	doctor_id = models.IntegerField()

	def __str__(self):
		return 'Appointment ID: %s' % (self.appointment_id)